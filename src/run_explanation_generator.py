...
from combination_regulations import combine_tuple, rem_sublist, rem_sublist_2, combine_triple_2, combine_triple_1, \
    sublist, filter_concept
from explanation_utils import write_txt
from lime_text import LimeTextExplainer
from model_utils import classify_label, classify_prob, normalize, ontology_based_weights, norm_min_max, \
    score_funct, stack_info_pl
from ontology_sampling_utils import normal_sampling, prime2word, occurrences, sampling_process, ontology_based_sample_z
from text_processing_utils import raw_tweet_change_punc, rem_punc, is_verb, is_adj, is_prep, w_type, change_format, \
    check_concepts, concatenate_list_data


def vectorize_ontology():
    ...
    onto_vec = []
    for _i in ontology_list:
        onto_vec.append(stack_info_pl(_i, tokenizer, w2v_model))
    _ontology = [ontology_list, ontology_idx, onto_vec]
    ...
    for _i in ontology_list:
        onto_vec.append(stack_info_pl(_i, tokenizer, w2v_model))
    _ontology_noStem = [ontology_list, ontology_idx, onto_vec]
    return _ontology, _ontology_noStem


def ontology_reasoning(target_idx, target_tweet_ori, target_tweet, target_tweet_stem, target_label, actual_label,
                       coarse_tweet, rem_conj_tweet, rem_conj_tweet_remPunc, learned_w_dict, local_vocab_dict,
                       result_path, model, ontology, in_onto_concepts_stem, in_onto_concepts, abstract,
                       isolated_concept, tuples_list_all, rem_posistion, concept_assign_dict, pos, issue_text,
                       cc_property, anchor_position, position_list, record, accuracy, rn_vect):
        ...
        if len(tuples_list_all) == 0:  # No tuples found in the tweet
            ...
            for tuple_idx_1 in tuples_list_all:
                ...
                if len(tuple_idx) == 1:  # only one tuple
                    ...
                    for i1 in range(len(tuple_idx) - 1):
                        for i2 in range(i1 + 1, len(tuple_idx)):
                            ...
                            if len(intersect) > 0:
                                ...
                                if (p_t1_s == p_t2_s) and (
                                        p_t1_s == [0]):  # Case 1: AB & AC => A {B and/or C} if possible
                                    tuples_list_tmp = combine_tuple(tuples_list_tmp, pos, t1, t2, in_onto_concepts,
                                                                    w_tk, concept_assign_dict, 1)
                                elif (p_t1_s == p_t2_s) and (
                                        p_t1_s == [1]):  # Case 2: AC & BC => {A and/or B} C if possible
                                    tuples_list_tmp = combine_tuple(tuples_list_tmp, pos, t1, t2, in_onto_concepts,
                                                                    w_tk, concept_assign_dict, 0)
                    ...
                    tuples_list_tmp = [list(x) for x in
                                       set(tuple(x) for x in tuples_list_tmp)]  # remove repetition
                ...
                if len(tuple_idx) == 1:
                    ...
                    for i1 in range(len(tuple_idx) - 1):
                        for i2 in range(i1 + 1, len(tuple_idx)):
                            ...
                            intersect = list(set(t1) & set(t2))
                            if len(intersect) == 2:
                                tuples_list = combine_triple_2(tuples_list, t1, t2, intersect, in_onto_concepts, w_tk,
                                                               concept_assign_dict, pos)
            ...
            for remove_idx in tuples_list:  # already change tp correct calculation for probability
                rest_idx = [i for i in range(len(coarse_tweet_tk)) if
                            i not in remove_idx and coarse_tweet_tk[i] in vocab_cc]
                rest_words = [coarse_tweet_tk[i] for i in range(len(coarse_tweet_tk)) if
                              i in rest_idx and coarse_tweet_tk[i] in vocab_cc]
            ...
            limit_rule = 5
            rules_OLLIE, rules, ollie_anchor_allPos_, ensemble_ollie_osil_anc, tuples_list, ollie_anchor_pos_ind, ensemble_ind = gen_OLLIE3(
                target_idx, coarse_tweet, model, target_label, ori_prob_vec, max_length, vocab_cc, rules, w_tk,
                anchor_position, anchor_idx, position_list, tuples_list, tuples_vis_list, osil_pos, limit_rule)
            ...
            w_LIME_20 = [w_LIME_20]
            rules_onto, rule_ollie, rule_ensemble = write_txt(all_explanation, sorted_w_and_i, sorted_norm_w_and_i,
                                                              target_idx, target_tweet_ori, target_label, actual_label,
                                                              result_path, rules_OLLIE, rules, ollie_anchor_allPos_,
                                                              anchor_only, onto_only, ensemble_ollie_osil_anc,
                                                              w_LIME_20)
            ...
            exp2.save_to_file(
                '../result/cc_' + str(target_idx) + '_label' + str(target_label) + '_rn' + str(rn) + '.html',
                rules_onto, rule_ollie, rule_ensemble, iss_name, rn)
            ...
            for r in range(no_r):
                ...
                if r == 0:  #### Case 1 rule removed ##
                    ...
                    rest_LIME = [coarse_tweet_tk[w] for w in range(len(coarse_tweet_tk)) if
                                 w not in in_idx and coarse_tweet_tk[w] in vocab_cc]
                    ...
                    accuracy.append(
                        [target_idx, diff_score_OSIL, diff_score_LIME, diff_score_OLLIE, diff_score_ensemble])
                else:
                    # OSIL
                    ...
                    rest_words_ = [coarse_tweet_tk[w] for w in range(len(coarse_tweet_tk)) if
                                   w in rest_idx and coarse_tweet_tk[w] in vocab_cc]
                    x_tmp = [vocab_cc[k] for k in rest_words_]
                    rest_vector_ = pad_sequences([x_tmp], maxlen=max_length, padding='post',
                                                 truncating='post')  # truncate the post, and keep pre
...
def gen_OLLIE3(target_idx, coarse_tweet, model, target_label, ori_prob_vec, max_length, vocab_cc, rules_OSIL, w_tk,
               anchor_position, anchor_idx, position_list, tuples_list, tuples_vis_list, osil_pos, limit_rule):
    # Add OLLIE rules
    f = open("../data/OLLIE_cc_out_unpack/out" + str(target_idx), "r")
    a = []
    for line in f:
        a.append(line)
    # ollie_triples = []
    ...
    ensemble_ollie_osil_anc = [ensemble_ollie_osil_anc[i] for i in range(len(ensemble_ollie_osil_anc)) if
                               i not in Ensem_0]
...
def explain_classifier_tf_count(target_idx, target_tweet, coarse_tweet, actual_label, stopword, cause_list, join_list,
                                abstract_concepts, model, ontology, params, result_path, concept_assign_dict,
                                vocab_matrix, vocab_cc, max_length, issue_text, cc_property, record, accuracy,
                                accuracy2, accuracy3, rn_vect):
    '''
    Inputs:
    ...
    z_strs_list, z_primes_list, in_onto_concepts_stem, in_onto_concepts, \
        abstract, isolated_concept, tuples_list, target_label, target_vector, \
        local_vocab_list, local_vocab_dict, target_tweet_remPunc, target_tweet_stem_remPunc, rem_conj_tweet, \
        rem_conj_tweet_remPunc, rem_posistion, anchor_position, position_list = ontology_based_sample_z(target_tweet,
            pos_target_tweet, ontology, abstract_concepts, model, cause_list,
            concept_assign_dict, vocab_cc, max_length,
            local_fid=params['local_fid'], no_repeat=params['no_repeat'],
            sample_normal=params['sample_normal'], sample_onto=params['sample_onto'],
    ...
    for i in range(num_samples):  # create num_samples of samples
        ...
        x_tmp = [vocab_cc[k] for k in sen_tk]
        z_vector = pad_sequences([x_tmp], maxlen=max_length, padding='post',
                                 truncating='post')  # truncate the post, and keep pre
    ...
    with the_graph.as_default():
        # label
        T_f_z = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1])
        # features
        T_z_prime = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, len_real_vocab])
        # sample weights
        T_sample_weight = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1])
        # coef to estimate
        T_w = tf.Variable(initial_value=tf.random.uniform([len_real_vocab]), dtype=tf.float32)
        # intercept to estimate
        T_b = tf.Variable(initial_value=tf.random.uniform([1]), dtype=tf.float32)
        # compute g(z') for each tweets
        T_g_z_all = T_z_prime * T_w  # + T_b # PL: Add T_b
        T_g_z_prime = tf.reduce_sum(input_tensor=T_g_z_all, axis=1)  # shape=[None, explain_size]
        T_g_z_prime = tf.expand_dims(T_g_z_prime, axis=1)

        loss = tf.square(T_sample_weight * (T_f_z - T_g_z_prime))
        loss_sum = tf.reduce_sum(input_tensor=loss)

        if reg == 'L0':
            l0_regular = tf.math.count_nonzero(T_w)
            ...
            alpha = params['l1_reg']
            total_loss = loss_sum + alpha * tf.reduce_sum(input_tensor=l1_regular)
        else:
            total_loss = loss_sum

        if method_train == 'Gradient':
            # Train optimizer using Gradient
            opt = tf.compat.v1.train.GradientDescentOptimizer(params['learning_rate'])
            gradients = opt.compute_gradients(total_loss)
            train_step = opt.apply_gradients(gradients)
        elif method_train == 'Adam':
            # Train optimizer using Adam 
            train_step = tf.compat.v1.train.AdamOptimizer(params['learning_rate']).minimize(total_loss)
    ...
    with tf.compat.v1.Session(graph=the_graph) as sess:
        ...
        for i in range(int(params['num_epoch'])):
            ...
            _, _, _, _, _, _, _, _ = sess.run(
                [total_loss, T_f_z, T_z_prime, T_sample_weight, T_w, T_b, T_g_z_all, T_g_z_prime],
                feed_dict={T_f_z: step_f_z,
                           T_z_prime: step_z_primes,
                           T_sample_weight: step_sample_weights})
            # print(total_loss_)
    ...
    accuracy, accuracy2, accuracy3, rn_vect = ontology_reasoning(target_idx, target_tweet, target_tweet_remPunc,
                                                                 target_tweet_stem_remPunc, target_label, actual_label,
                                                                 coarse_tweet, rem_conj_tweet, rem_conj_tweet_remPunc,
                                                                 learned_w_dict, local_vocab_dict, result_path, model,
                                                                 ontology, in_onto_concepts_stem, in_onto_concepts,
                                                                 abstract, isolated_concept, tuples_list, rem_posistion,
                                                                 concept_assign_dict, pos_target_tweet, issue_text,
                                                                 cc_property, anchor_position, position_list, record,
                                                                 accuracy, rn_vect)
    print('Great job!')
    return accuracy, accuracy2, accuracy3, rn_vect
...
if __name__ == '__main__':
    ...
    print('Loading and vectorizing ontology')
    ontology, ontology_noStem = vectorize_ontology()

    print('Reading custom stopwords')
    ...
    print('Reading concept properties')
    ...
    for i in no_test:
        ...
        if len_tweet <= local_fid + 1:
            ...
            if len_tweet > local_fid:
                num_samples = len_tweet * (
                            len_tweet - local_fid) * no_repeat
            else:
            ...
            params = {'explain_size': explain_size,
                      ...
                      'min_sample_prob': 0.005,  # minimum sample prob for sampling x
                      'sample_prob_decay': 0.9,
                      # the rate that sample probability decays when tweets are less similar (x)
                      ...
                      'kernel_width': 25,  # kernel width parameter
                      'metric': 'cosine',  # 'cosine' or 'euclidean'
                      'cosine_scale': 100  # scale to enlarge cosine distance values for kernel function
                      }

            result_path = '../result/CC_Mortgage_check.txt'

            accuracy, accuracy2, accuracy3, rn_vect = explain_classifier_tf_count(i, target_tweet, coarse_tweet,
                                                                                  actual_label, stopword, cause_list,
                                                                                  join_list, abstract_concepts, model,
                                                                                  ontology, params, result_path,
                                                                                  concept_assign_dict, vocab_matrix,
                                                                                  vocab_cc, max_length, issue_text,
                                                                                  cc_property, record, accuracy,
                                                                                  accuracy2, accuracy3, rn_vect)
            if cnt % 10 == 0:
                np.savez('rn_vect' + str(cnt) + '.npz', rn_vect=rn_vect)
                np.savez('record_all_cc_' + str(cnt) + '.npz', accuracy=accuracy, accuracy2=accuracy2,
                         accuracy3=accuracy3)
            print('------------ ******** ------------')
